...
<?php 
  ...
  */
    ...
    $chemical_res = select("SELECT * FROM `equipment` WHERE `id`=? AND  `status`=?",[$data['id'],1],'ii');
...
?>
    ...
    <select class='form-select shadow-none' aria-label='Default select example' name='teacher' required>
        ...
        ?>
                  ...
                  <button name="pay_now_equipment" type="submit" class="btn btn-success w-100 text-white shadow-none mb-1" disabled>Borrowing Now</button>
                  </div>
                </div>
            </form>
            </div>
...
function check_availability(){
   ...
   booking_form.elements['pay_now_equipment'].setAttribute('disabled',true);
   ...
   if(checkin_val!='' && checkout_val!=''){
      ...
      let xhr = new XMLHttpRequest();
      xhr.open("POST","./ajax/confirm_equipment.php",true);
